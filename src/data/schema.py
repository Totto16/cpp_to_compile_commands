...
#   timestamp: 2025-05-27T13:26:19+00:00
...
class Browse(BaseModel):
    ...
    databaseFilename: Optional[str] = Field(
        None,
...
class Configuration(BaseModel):
    ...
    cStandard: Optional[CStandard] = Field(
        None,
    ...
    cppStandard: Optional[CppStandard] = Field(
        None,
        description="Version of the C++ language standard to use for IntelliSense. Note: GNU standards are only used to query the set compiler to get GNU defines, and IntelliSense will emulate the equivalent C++ standard version.",
    )
    compileCommands: Optional[Union[str, List[str]]] = None
    includePath: Optional[List[str]] = None
    macFrameworkPath: Optional[List[str]] = Field(
        None,
    ...
    windowsSdkVersion: Optional[
        constr(regex=r"^\d{2}\.\d{1}\.\d{5}\.\d{1}$|^8\.1$")
    ] = None
    dotConfig: Optional[str] = Field(
        None,
    ...
    customConfigurationVariables: Optional[Dict[constr(regex=r"(^.+$)"), str]] = None
...
class Env(BaseModel):
    __root__: Dict[
        constr(
            regex=r"(?!^workspaceFolder$)(?!^workspaceRoot$)(?!^workspaceFolderBasename$)(?!^execPath$)(?!^pathSeparator$)(?!^default$)(^.+$)"
        ),
        Union[str, List[str]],
    ]
...
class EnableConfigurationSquiggles(BaseModel):
    __root__: bool
